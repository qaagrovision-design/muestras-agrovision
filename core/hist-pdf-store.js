...
(function histPdfStoreModule() {
    ...
    async function borrarPorModulo(modulo) {
        const mod = String(modulo || '').trim().toLowerCase();
        if (!mod || !window.indexedDB) return;
        const todos = await listarTodos();
        const ids = todos
            .filter((r) => String(r?.modulo || '').trim().toLowerCase() === mod)
            .map((r) => r.id)
            .filter(Boolean);
    ...
    async function modulosDisponibles(fecha, ensayoNumero, numMuestra) {
        ...
        for (const m of ['campo', 'acopio', 'packing', 'packing-rc5', 'mptk', 'tk-2.0']) {
