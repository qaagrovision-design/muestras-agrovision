...
(function campoPdfModule() {
    ...
    function logoUrlCampoPdf_() {
        return urlCampoPdfDesdeRaiz_('images/log.png');
    }
    ...
    async function regenerarPdfVivoSiAbiertoCampo() {
        if (!window.PdfPreviewLive?.modalAbierto?.()) return;
        if (typeof window.obtenerDatosPdfCampo !== 'function') return;
        try {
            if (!obtenerJsPDF()) return;
            const datos = window.obtenerDatosPdfCampo();
            const hash = JSON.stringify(datos);
            if (hash === ultimoHashPdfVivoCampo_) return;
            ultimoHashPdfVivoCampo_ = hash;
    ...
    async function abrirModalPdf(blob, nombre, datosPdf) {
        ...
        ultimoHashPdfVivoCampo_ = datosPdf ? JSON.stringify(datosPdf) : '';
        actualizarTituloModalPdf_(pdfNombreActual, datosPdf);
        const ov = document.getElementById('pdf-modal-overlay');
        if (!ov) return;
        revocarPdfUrlActual();
    ...
    function initPdfVisorModalCampo_() {
        ...
        window.PdfPreviewLive?.registrar?.(regenerarPdfVivoSiAbiertoCampo);
