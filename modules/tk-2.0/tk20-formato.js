/** Selector MP-TK / TK-2.0 (misma UX; navegación entre módulos distintos). */
(function initTk20FormatoSelect() {
    const MPTK_FORMATO_KEY = 'tiempos-mptk-formato-v1';
    const sel = document.getElementById('mptk-formato');
    if (!sel) return;

    const paginaActual = String(document.body.dataset.mptkFormato || '2.0').trim();
    const urlMpTk = sel.dataset.urlMpTk || '../mp-tk/';
    const urlTk20 = sel.dataset.urlTk20 || './';
    ...
    try {
        ...
        // Recordar formato actual: al volver desde Campo vía tab MP-TK cae en TK-2.0.
        localStorage.setItem(MPTK_FORMATO_KEY, paginaActual === '2.0' ? '2.0' : (sel.value || paginaActual));
    } catch (_) {
